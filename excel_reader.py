...
class ExcelReader(CalculationParsingMixin, ResponseParsingMixin):
    """Reads a `_dd` worksheet into a list of `Question`, validating as it goes.

    The two block formats a Responses cell can hold are parsed by mixins:
    `calc:` by `CalculationParsingMixin` (`calculation_parser.py`) and
    `source:` by `ResponseParsingMixin` (`response_parser.py`). They are
    inherited rather than delegated to so every method stays reachable at the
    attribute path it had before the split -- which matters for
    `_validate_calculation_fields` in particular, since
    `tests/test_calculation_registry.py` reads its source off
    `ExcelReader` to check that every `CalculationType` has a branch.

    The mixins carry no state of their own. They read `_error`, `_split_lines`,
    `logstring` and (for the calculation half) `DATE_RANGE_RE` off `self`, the
    same object they read them off before.
    """
